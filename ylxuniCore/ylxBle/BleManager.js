import {dataTypeJudge, debounce} from "@/utils/tools";
import {extractHexType} from "@/ylxuniCore/ylxBle/business";
import {BLENotificationBatchEnabler} from "@/ylxuniCore/ylxBle/BLENotificationBatchEnabler";
...
let processingLock = Promise.resolve(); // 新增Promise链锁
...
export class BleManager {
    ...
    constructor({advertisServiceUUIDs = [], name = '', serviceId = ''}) {
        ...
        BleManager.enabler = new BLENotificationBatchEnabler({
            characteristics: [],
            batchSize: 1,
            delay: 180,
        });
        ...
        this.startDeviceSearchTimeout = null
        ...
        return new Promise((resolve, reject) => {
            uni.getBluetoothDevices({
                success: (res) => {
                    ...
                    const bluetoothDevices = BleManager.name
                        ? res.devices.filter(item => item.name === BleManager.name)
                        : res.devices;
    ...
    onBluetoothDeviceFound(callback) {
        uni.onBluetoothDeviceFound((res) => {
            ...
            if (typeof callback === 'function' && bluetoothDevices.length) {
                debounce(callback, 300)(bluetoothDevices);
            }
    ...
    onBLEConnectionStateChange(callback) {
        if (typeof callback === 'function') {
            uni.onBLEConnectionStateChange((data) => {
                console.warn('notifyList', notifyList?.[data.deviceId]);
                if (!data.connected && notifyList.hasOwnProperty(data.deviceId)) {
                    console.warn('222-notifyList', notifyList?.[data.deviceId]);
                    delete notifyList?.[data.deviceId]
                    console.error('333-notifyList', notifyList?.[data.deviceId]);
                }
    ...
    // 启用所有支持 Notify 的特征
    enableNotification(deviceId, characteristics, callback, errCallback, delay = 230) {

        let num = 0
        let maxMum = 10

        function enableNotify(index, deviceId) {
    ...
    handleWrite({paramHex, deviceId, characteristicId, serviceId}, resolve, reject) {
        ...
        enableCharacteristicIdNotify({deviceId, characteristicId, serviceId}, () => {
            ...
            const loopWrite = () => {
                ...
                if (plus.os.name == "iOS") {
                    onIOSPromiseFn[deviceId] = extractHexType(characteristicId)
                ...
                console.warn('发送命令=====>', {paramHex, characteristicId: extractHexType(characteristicId), deviceId, serviceId,})
                ...
                uni.writeBLECharacteristicValue({
                    ...
                    fail: (fail) => {
                        console.error('写入失败', {paramHex, characteristicId: extractHexType(characteristicId), deviceId}, fail)
        ...
        uni.onBLECharacteristicValueChange(res => {
            ...
            if (this.isUpgrade) {
                processChain = processChain.then(async () => {
                    ...
                    if (plus.os.name == "iOS") {
                        if (extractHexType(characteristicId) === '34') {
                            await onDataCallback({hexString, deviceId, serviceId, characteristicId})
                        } else {
                            await onDataCallback({hexString, deviceId, serviceId, characteristicId})
                        }
                ...
                if (plus.os.name == "iOS") {
                    if (extractHexType(characteristicId) === '34') {
                        ...
                        // onDataCallback({hexString, deviceId, serviceId, characteristicId})
                    } else {
                        onDataFoundCallback({hexString, deviceId, serviceId, characteristicId})
                        // this.onData({hexString, deviceId, serviceId, characteristicId})
                        setTimeout(() => {
                            foundCallback({hexString, deviceId, serviceId, characteristicId})
                            // onDataCallback({hexString, deviceId, serviceId, characteristicId})
                        }, 150)
